...
  transactions: any[]; // Kept for initialization logic
}
...
  }, [budgets]);
  ...
  // Removed Monthly Reset Logic useEffect entirely, as App.tsx handles the monthly reset now.

  // Removed getBudgetWithSpending function entirely.

  // Use the budgets array received from the parent directly, as it contains the calculated 'spent'
  const budgetsForDisplay = budgets; 
  
  // No recalculation needed here: budgetsWithSpending = budgets.map(getBudgetWithSpending);
...
      budgeted: parseFloat(budgetAmount),
      // IMPORTANT: When saving an edit/add, reset spent to 0 if new, 
      // or rely on the parent's current monthly calculation.
      // Since App.tsx calculates spent, we only need to preserve lastReset/id/budgeted/category/color. 
      // We pass 0 for spent, knowing App.tsx will recalculate it immediately anyway.
      spent: 0, 
      color: selectedColor,
...
        b.id === editingBudget.id ? { ...newBudget, spent: b.spent } : b // Keep calculated spent for immediate local use
      );
